...
A live status page for the capture rig described on The Model page. The rig publishes a JSON status file to a public GitHub gist every two minutes; the page fetches it on load and every 60 seconds:

```
GET https://api.github.com/gists/0f7aaf406781706fee444a9ad8e94103   -> files["status.json"].content (a JSON string)
fallback: https://gist.githubusercontent.com/sbrunner-atx/0f7aaf406781706fee444a9ad8e94103/raw/status.json
```
